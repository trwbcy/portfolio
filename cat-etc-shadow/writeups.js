...
const writeups = [
    ...
    { title: "ClashOrCrash", type: "red", desc: "Cookie Forgery BAC + Stored CMDi + DirtyFrag PrivEsc", url: "https://drive.google.com/file/d/1hfeO5Z5gveRmi_MGnTr7q8sOg7osc2qh/view?usp=sharing" },
    ...
    { title: "E-LEET", type: "red", desc: "wpDiscuz RCE (CVE-2020-24186) + Password Reuse → Root", url: "https://drive.google.com/file/d/1_jVqCQ-AAvr1zspGfVlTF0xt65RdW-15/view?usp=drive_link" },
...
function updateStats() {
    ...
    const off = writeups.filter(w => w.type === 'red').length;
    const def = writeups.filter(w => w.type === 'blue').length;

    document.getElementById('statTotal').innerHTML = total;
    document.getElementById('statOff').innerHTML = `${off} <em>ROOTED</em>`;
